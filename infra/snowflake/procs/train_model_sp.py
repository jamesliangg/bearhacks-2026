...
def main(session, algo: str = "gbr") -> str:
    """Snowflake Python stored procedure entrypoint.

    Trains a simple model and stores it to @MART.MODEL_STAGE and updates MART.MODEL_RUNS.
    ...
    # Feature engineering (minimal parity with via_common.features):
    # target = max(delay_minutes) per train/day
    # features: dow, month, hour, is_weekend, train_number_hash, avg_delay_l30d, avg_delay_l30d_dow
    if obs.empty:
    ...
    per_day = (
        ...
        .agg(
            ...
            hour=("SCHEDULED_ARRIVAL", lambda s: int(pd.to_datetime(s, errors="coerce").dt.hour.dropna().iloc[0]) if s.notna().any() else 12),
        )
    ...
    # Rolling stats within each train number.
    ...
    # Train (keep it simple for SP runtime; sklearn may not be available).
    # Use a very simple linear-ish fit with numpy (ridge).
